...
This post is a literate haskell file, so that means you can load it up directly
in GHC and play around with it! Let's get started.
...
amongst possible options while still maintaining a focus. It does this by
using an infinitely recursive structure which wraps around a given functor (in
our case TPossible). Let's build up a few of these structures by combining
...
describe function to 'tabulate' which returns the value we want to appear at any
given index. Remember, the index we chose for TPossible is TChoice. The index for
Cofree TPossible is a Sequence of TChoice! 

Let's build our first actual 'Tape' using tabulate with our Cofree
Representable instance! Here's an infinite number-line going out in both directions from our focus:
...
Great! Extend works by duplicating the Comonad meaning we'll have a (Cofree
TPossible (Cofree TPossible a)), then it fmaps over the duplicated parts with
...
Comonads, Cofree, and zippers! If you have any questions find me on twitter
@chrislpenner
